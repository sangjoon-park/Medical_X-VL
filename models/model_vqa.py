...
class ALBEF_original(nn.Module):
    def __init__(self,
                 text_encoder=None,
                 text_decoder=None,
                 tokenizer=None,
        ...
        visual_encoder = VisionTransformer_deit(
            img_size=config['image_res'], patch_size=16, embed_dim=384, depth=12, num_heads=6,
        ...
        if self.distill:
            visual_encoder_m = VisionTransformer_deit(
                img_size=config['image_res'], patch_size=16, embed_dim=384, depth=12, num_heads=6,
                mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))
            self.visual_encoder_m = visual_encoder_m
            self.text_encoder_m = BertModel(config=config_encoder, add_pooling_layer=False)
