...
class XVLModel(nn.Module):
    ...
    def forward(self, it, images, text, masks, label, ibot_loss, epoch, fp16_scaler, alpha=0):
        bs = images[0].size(0)
        ...
        attention_weights = output_pos.attentions[-1].clone()
        attention_weights = attention_weights.mean(dim=1)
        attention_weights = attention_weights.mean(dim=1)

        with torch.no_grad():
            dataframe = self.data_loader.dataset.train_df
        ...
        answer_targets = labels.masked_fill(labels == self.tokenizer.pad_token_id, -100)

        answer_output = self.text_encoder(input_ids,
                                          attention_mask=text.attention_mask,
                                          encoder_hidden_states=image_embeds,
                                          encoder_attention_mask=image_atts,
        ...
        loss_mlm = answer_output.loss
        loss_mlm = loss_mlm.sum() / bs * 0.1
