...
from health_multimodal.image.data.transforms import create_chest_xray_transform_for_inference, \
    create_chest_xray_transform_for_train
...
class CXRTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = self._resize_img(image, 320)
...
def predict(loader, model, positive, class_names, template, softmax_eval=True, verbose=0):
    """
    FUNCTION: predict
    ---------------------------------
...
def run_single_prediction(cxr_labels, template, model, positive, loader, softmax_eval=True, context_length=120):
    """
    ...
    y_pred, y_true = predict(loader, model, positive, cxr_labels, template, softmax_eval=softmax_eval)
...
def run_softmax_eval(model, loader, eval_labels: list, pair_template: tuple, context_length: int = 120):
    """
    ...
    pos_pred, y_true = run_single_prediction(eval_labels, pos, model, 1, loader,
                                     softmax_eval=True, context_length=context_length)
    neg_pred, y_true = run_single_prediction(eval_labels, neg, model, 0, loader,
                                     softmax_eval=True, context_length=context_length)
...
def run_experiment(model, cxr_labels, cxr_templates, loader, alt_labels_dict=None, softmax_eval=True,
                   context_length=120, use_bootstrap=True):
    '''
...
def run_zero_shot(cxr_labels, cxr_templates, model_path, cxr_filepath, final_label_path, alt_labels_dict: dict = None,
                  softmax_eval=True, context_length=120, pretrained: bool = False, use_bootstrap=True, cutlabels=True):
    """
    ...
    results, y_pred = run_experiment(model, cxr_labels, cxr_templates, loader,
...
def run_cxr_zero_shot(model_path, context_length=120, pretrained=False):
    ...
    cxr_labels = ['Atelectasis', 'Cardiomegaly', 'Edema', 'Consolidation', 'Fracture', 'Pleural Effusion', 'Pneumonia',
                  'Pneumothorax']
